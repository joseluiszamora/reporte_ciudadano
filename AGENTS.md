...
El repositorio contiene documentación en `docs/` y un prototipo Flutter para Android hasta la novena entrega: tema, navegación, lista/detalle públicos, acceso simulado, reportes y borradores, moderación con versiones, ocultamiento, duplicados, confirmaciones únicas, aportes revisados, seguimiento, avisos simulados, gestiones revisadas, solución/verificación/reapertura, lectura pública compartida, retiro de autoría, denuncia de contenido, mapa esquemático con filtros, minimapa y punto ajustable. La moderación editorial y el ocultamiento operan sobre reportes creados en el dispositivo; el catálogo es de lectura y admite participación, gestiones, decisiones de seguimiento y recepción/revisión de denuncias sin ocultarlo. Consulta `README.md` y `docs/09_entrega_ubicacion_validacion.md` para el alcance, ejecución y comprobaciones actuales; las entregas anteriores se conservan en los documentos 03–08. No hay infraestructura ni integraciones externas reales; no describas componentes planeados como existentes.

La etapa en curso es un prototipo navegable en Flutter, en español y orientado a Android, con datos ficticios y repositorios sustituibles. Los diez recorridos de demostración tienen cobertura automatizada y se ejecutaron manualmente en un teléfono Android físico; consulta los resultados y límites en `docs/09_entrega_ubicacion_validacion.md`. La auditoría con lector de pantalla sigue pendiente. Construir el prototipo no autoriza contratar infraestructura ni abrir un piloto real.
...
- La séptima entrega usa JSON v5, compatible con v1–v4, y `SafetyRepository`/`SafetyState`. El retiro pertenece al autor de un reporte aprobado y anonimiza sus metadatos y sus aportes en ese reporte, incluidas revisiones futuras; conserva aportes de terceros, contenido y vínculo interno. No depura datos personales escritos en campos libres ni elimina cuentas. Denuncias privadas, idempotentes, consultables por denunciante/equipo; el denunciante no recibe motivos internos ni identidad del revisor. Resolver y ocultar un reporte local se guardan juntos. No se permite ocultamiento del catálogo ni retirada granular de un comentario/foto en este recorrido.
...
La octava entrega mantiene JSON v5. `ExploreFilter` centraliza categoría, seguimiento, fecha de última observación en Bolivia, búsqueda y área; lista y mapa comparten resultados públicos. `DemoReportMap` es una representación esquemática sustituible, con agrupación por celdas y controles accesibles. La novena añade `SchematicPointMap` en detalle y envío; tocar el plano o usar flechas actualiza el borrador y conserva la entrada manual. `LocationAdapter` continúa simulado y se solicita en contexto. No hay calles, límites oficiales ni proveedor contratado.
