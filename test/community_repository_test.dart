...
void main() {
  ...
  Future<String> publish() async {
    ...
    final draft = repo.createDraft().copyWith(
      ...
      description:
          'Descripción ficticia para pruebas de participación ciudadana.',
      latitude: '-16.5000',
      longitude: '-68.1600',
      cityConfirmed: true,
    );
  ...
  Future<Contribution> send(
    String reportId, {
    ContributionKind kind = ContributionKind.evidence,
    String body = 'Evidencia ficticia del problema urbano',
  }) async {
    final draft = (await repo.createContribution(reportId))
        .copyWith(body: body, kind: kind, photos: ['Adjunto privado simulado']);
  ...
  test('confirmación única excluye autor y notifica solo al cruzar umbral propuesto', () async {
    ...
    await Future.wait([
      repo.confirmObservation(id, now),
      repo.confirmObservation(id, now),
    ]);
  ...
  test(
    'aporte pendiente privado, corrección, publicación y aviso son atómicos',
    () async {
      final id = await publish();
      await repo.follow(id, true);
      await citizen(DemoProvider.google);
      ...
      final draft = (await repo.createContribution(
        id,
        previousId: first.id,
      )).copyWith(body: 'Aporte corregido y ficticio');
      await repo.sendContribution(draft);
      session.enterModerationScenario();
      await repo.reviewContribution(
        draft.id,
        ReviewStatus.approved,
        'Motivo interno confidencial ficticio',
      );
      final report = (await public.getPublicReport(id))!;
  ...
  test('edición pendiente y rechazada de aporte conserva texto y adjuntos públicos', () async {
    ...
    await citizen(DemoProvider.google);
    final edit = (await repo.createContribution(
      id,
      previousId: first.id,
    )).copyWith(body: 'Texto privado nuevo', photos: ['Adjunto nuevo privado']);
    await repo.sendContribution(edit);
    expect(
      (await public.getPublicReport(id))!.updates.single.text,
      isNot(contains('nuevo')),
    );
    session.enterModerationScenario();
    await repo.reviewContribution(
      edit.id,
    ...
    expect(
      (await public.getPublicReport(id))!.updates.single.text,
      contains(first.draft.body),
    );
    await citizen(DemoProvider.google);
    await expectLater(
      repo.createContribution(id, previousId: first.id),
      throwsStateError,
    );
    ...
    expect(
      (await public.getPublicReport(id))!.updates.single.text,
      contains('Texto privado nuevo'),
    );
  });

  test(
    'ocultamiento retira aportes, conteos, seguimientos y avisos públicos',
    () async {
      final id = await publish();
      await repo.follow(id, true);
      await citizen(DemoProvider.google);
      ...
      expect(
        (await public.listPublicReports()).any((r) => r.id == id),
        isFalse,
      );
      await citizen();
  ...
  test(
    'aprobar aporte con reporte oculto no envía aviso público ni lo restaura',
    () async {
      final id = await publish();
      await repo.follow(id, true);
      await citizen(DemoProvider.google);
  ...
  test('comentarios optativos y agrupados, lectura y push rechazado no bloquean bandeja', () async {
    ...
    for (var i = 0; i < 3; i++) {
      if (i == 1) {
        ...
        await repo.savePreferences(
          const NoticePreferences(comments: true, pushDenied: true),
        );
      }
      await citizen(DemoProvider.google);
      final item = await send(
        id,
        kind: ContributionKind.comment,
        body: 'Comentario ficticio $i',
      );
      session.enterModerationScenario();
      await repo.reviewContribution(item.id, ReviewStatus.approved, 'Revisado');
      await citizen();
      expect(
        (await repo.inbox()).where((n) => n.kind == NoticeKind.comment),
        hasLength(i == 0 ? 0 : 1),
      );
    }
    ...
    expect(
      (await repo.inbox())
          .where((n) => n.kind == NoticeKind.comment)
          .single
          .read,
      isTrue,
    );
  ...
  test('reintento con respuesta perdida y fallo de disco no duplican ni publican parcialmente', () async {
    ...
    await repo.follow(id, true);
    await citizen(DemoProvider.google);
    final draft = (await repo.createContribution(id)).copyWith(
      kind: ContributionKind.evidence,
      body: 'Evidencia ficticia guardada',
    );
    await expectLater(
      repo.sendContribution(draft, scenario: SendScenario.offline),
      throwsStateError,
    );
    expect(repo.ownContributionDrafts.single.id, draft.id);
    await expectLater(
      repo.sendContribution(draft, scenario: SendScenario.lostResponse),
      throwsStateError,
    );
    ...
    storage.fail = true;
    await expectLater(
      repo.reviewContribution(sent.id, ReviewStatus.approved, 'Aprobado'),
      throwsStateError,
    );
    ...
    await expectLater(
      repo.reviewContribution(sent.id, ReviewStatus.rejected, 'Rechazado'),
      throwsStateError,
    );
    await citizen();
    expect((await repo.inbox()).where((n) => !n.isPrivate), hasLength(1));
  ...
  test(
    'persistencia v3 conserva cuentas, observaciones, borradores y avisos',
    () async {
      final id = await publish();
      await repo.follow(id, true);
      ...
      await citizen(DemoProvider.google);
      await repo.confirmObservation(id, now);
      final draft = (await repo.createContribution(id))
          .copyWith(body: 'Borrador ficticio conservado');
  ...
  test('migración v2 conserva publicaciones y añade comunidad vacía', () async {
    ...
    old['version'] = 2;
    old.remove('community');
    storage.value = jsonEncode(old);
    final reopened = LocalSubmissionRepository(
      session: session,
      storage: storage,
      clock: () => now,
  ...
  test(
    'visitante y moderador no participan; futuro y motivo vacío se rechazan',
    () async {
      final id = await publish();
      await expectLater(
        repo.confirmObservation(id, now.add(const Duration(days: 1))),
        throwsArgumentError,
      );
      final item = await send(id);
      await expectLater(
        repo.reviewContribution(item.id, ReviewStatus.approved, 'Propio'),
        throwsStateError,
      );
      session.signOut();
      expect(repo.ownContributions, isEmpty);
      ...
      await expectLater(repo.confirmObservation(id, now), throwsStateError);
      await expectLater(
        repo.reviewContribution(item.id, ReviewStatus.approved, ' '),
        throwsArgumentError,
      );
    },
  );
  ...
  test('confirmaciones del catálogo no degradan solución verificada', () async {
    ...
    expect(
      (await public.getPublicReport('4'))!.tracking,
  ...
  test('edición de reporte solo avisa al aprobar; aviso privado conserva destinatario', () async {
    ...
    await repo.submit(edit);
    await citizen(DemoProvider.google);
    expect(await repo.inbox(), isEmpty);
    session.enterModerationScenario();
    await repo.review(edit.id, ReviewStatus.approved, 'Motivo reservado');
    await citizen(DemoProvider.google);
    final notices = await repo.inbox();
    expect(notices, hasLength(1));
    expect(notices.single.isPrivate, isFalse);
    expect(notices.single.label, isNot(contains('Motivo reservado')));
    await citizen();
    expect(
      (await repo.inbox()).where((n) => n.targetId == edit.id).single.isPrivate,
      isTrue,
    );
  });
}
